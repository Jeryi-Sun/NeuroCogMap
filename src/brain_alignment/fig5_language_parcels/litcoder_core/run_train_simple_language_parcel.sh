#!/usr/bin/env bash
# 批量训练 language model parcel-level（narratives 数据集）
# 功能：
# 1) 循环处理 layer_idx（默认 1..25）
# 2) 支持 train/test split，并可选 evaluation split
# 3) 支持按 layer_idx 和已有结果跳过任务
set -euo pipefail
subject_id="${SUBJECT_ID:-uts249}"

data_name="${DATA_NAME:-narratives}"
dataset_type="${DATASET_TYPE:-narratives}"

PROJECT_ROOT="/path/to/project_root/Human_LLM_align/litcoder_core"
SCRIPT="${PROJECT_ROOT}/train_simple_language.py"
LOG_DIR="${PROJECT_ROOT}/logs/${subject_id}/language_parcel_level_$(date +%Y%m%d_%H%M%S)"
RESULTS_DIR_DEFAULT="${PROJECT_ROOT}/results/${subject_id}/results_language_parcel_level_$(date +%Y%m%d_%H%M%S)"
...
EVAL_STORY_FROM_END="${EVAL_STORY_FROM_END:-1}"
...
mkdir -p "${PROJECT_ROOT}/cache_language_model${data_name}" "${RESULTS_DIR}" "${LOG_DIR}"
...
PY
    then
      echo "跳过 layer_idx=${layer_idx}，结果文件已存在" | tee -a "${LOG_DIR}/batch_run_language_parcel.log"
      continue
    fi
  fi
  
  echo "开始处理 layer_idx=${layer_idx}, 日志文件: ${log_file}" | tee -a "${LOG_DIR}/batch_run_language_parcel.log"
  
  cmd=(python "${SCRIPT}" \
    --assembly_path "${ASSEMBLY_PATH}" \
    --layer_idx "${layer_idx}" \
    --cache_dir "${PROJECT_ROOT}/cache_language_model${data_name}" \
  ...
  if [ $? -eq 0 ]; then
    echo "layer_idx=${layer_idx} 处理完成" | tee -a "${LOG_DIR}/batch_run_language_parcel.log"
  else
    echo "layer_idx=${layer_idx} 处理失败，请查看日志: ${log_file}" | tee -a "${LOG_DIR}/batch_run_language_parcel.log"
  fi
done

echo "所有 layer_idx (1-25) 处理完成" | tee -a "${LOG_DIR}/batch_run_language_parcel.log"
