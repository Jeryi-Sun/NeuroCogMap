#!/usr/bin/env bash
# 批量提取 SAE Model 特征，一次性提取所有 Parcel (0-269)
...
LOG_DIR="${PROJECT_ROOT}/logs_9b/extract_sae_model_$(date +%Y%m%d_%H%M%S)"
RESULTS_DIR="${PROJECT_ROOT}/results_9b/cache_sae_model_pre_nozero_patch"
  ...
  found_all=true
    for parcel_id in {0..269}; do
      for participant_id in 0 1; do
        output_file="${RESULTS_DIR}/parcel_${parcel_id}/model=${MODEL_PATH//\//-}_extractor=sae_model_parcel_${parcel_id}_participant=${participant_id}.pth"
...
python "${SCRIPT}" \
  --model "${MODEL_PATH}" \
  --input "${INPUT_FILE}" \
  --extractor_type "sae_model_pre_nozero_patch" \
  --extract_all_parcels \
