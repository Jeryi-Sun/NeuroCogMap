...
# -*- coding: utf-8 -*-
"""
基于 SAE Activation 的 token 限制特征提取器
参考 saeact_model.py 的实现
提取的特征是每个token对应的 parcel latent 的激活值的平均值
"""
...
class SAEActModelTokenLimitedExtractor(BaseTokenLimitedExtractor):
    """基于 SAE 激活值的 token 限制特征提取器"""
    ...
    def extract_from_experiment(
        ...
        **kwargs
        ...
        Returns:
            提取的特征 tensor [num_activations, num_latents]
        """
        # 截断到 token 限制
        ...
        if not relevant_tokens.any():
            ...
            total_latents = sum(len(latents) for latents in self.layer_to_latents.values())
            if total_latents == 0:
                return torch.empty(0, 1, dtype=torch.float16)
            return torch.empty(0, total_latents, dtype=torch.float16)
        ...
        with torch.no_grad():
            ...
            for layer_id in sorted(self.layer_to_latents.keys()):
                ...
                # 只保留激活 token 的部分（确保 relevant_tokens 在正确的设备上）
                ...
                all_activation_vectors.append(sel_activations)
            
            if not all_activation_vectors:
                # 如果没有有效的激活向量，返回空 tensor
                total_latents = sum(len(latents) for latents in self.layer_to_latents.values())
                if total_latents == 0:
                    return torch.empty(0, 1, dtype=torch.float16)
                return torch.empty(0, total_latents, dtype=torch.float16)
            ...
            # 将所有层的激活值拼接起来
            final_vec = torch.cat(all_activation_vectors, dim=1)  # [num_activations, total_latents]
    ...
    def extract_all_parcels_from_experiment(
        ...
        **kwargs
        ...
        """
        从单个实验一次性提取所有 Parcel 的特征（激活值的平均值）
        ...
        Returns:
            字典，键为 Parcel ID，值为特征 tensor [num_activations, num_latents]
        """
        ...
        # 找到激活 token
        relevant_tokens = self.find_activation_tokens(tokenized_prompt.input_ids, only_left_token=only_left_token)
        ...
        if not relevant_tokens.any():
            ...
            total_latents = sum(len(latents) for latents in self.layer_to_latents.values())
            if total_latents == 0:
                return {parcel_id: torch.empty(0, 1, dtype=torch.float16) for parcel_id in parcel_ids}
            return {parcel_id: torch.empty(0, total_latents, dtype=torch.float16) for parcel_id in parcel_ids}
        ...
        with torch.no_grad():
            ...
            for parcel_id in parcel_ids:
                ...
                if not available_layer_to_latents:
                    ...
                    if total_latents == 0:
                        parcel_features[parcel_id] = torch.empty(0, 1, dtype=torch.float16)
                    else:
                        parcel_features[parcel_id] = torch.empty(0, total_latents, dtype=torch.float16)
                    continue
