...
# Parcel Intervention System 运行脚本
# 用于执行第三步的因果干预实验
...
check_dependencies() {
    ...
    # 检查合并后的最大激活文件
    if [ ! -f "$MAX_ACTIVATION_DIR/merged_max_activations.pkl" ]; then
        print_error "最大激活目录中没有找到合并后的激活文件: merged_max_activations.pkl"
        exit 1
    fi
...
run_single_dataset() {
    ...
    local log_file="$LOG_DIR/${dataset_name}_intervention.log"
    local timestamp=$(date '+%Y%m%d_%H%M%S')
    
    echo "=== 数据集 $dataset_name 干预实验开始 ($timestamp) ===" > "$log_file"
    
    python parcel_intervention.py \
...
run_all_datasets() {
    ...
    local log_file="$LOG_DIR/all_datasets_intervention.log"
    local timestamp=$(date '+%Y%m%d_%H%M%S')
...
# 显示帮助信息
show_help() {
    echo "Parcel Intervention System 运行脚本"
    echo ""
    echo "用法: $0 [选项]"
    echo ""
    echo "选项:"
    ...
    echo ""
    echo "示例:"
    echo "  $0 --dataset piqa --top-k 10    # 处理piqa数据集，取前10个parcel"
    echo "  $0 --all --top-k 5              # 处理所有数据集，每个取前5个parcel"
...
main() {
    ...
    DATA_DRIVEN_RESULTS="$PROJECT_ROOT/results/rank_activation/parcel_activation_rankings.json"
    LATENT_PARCEL_ASSIGNMENTS="/path/to/project_root/neural_area/divide_area_by_sae_act/cluster_output_2b_pt/clustering_results_sentence_prep0.03_0.8_svdvar0p80_parcels20_iter50_spatial0.01_nparcels270/latent_parcel_assignments.json"
    MAX_ACTIVATION_DIR="$PROJECT_ROOT/results/steer_activation"
    RESULTS_DIR="$PROJECT_ROOT/results/intervention/strength_-1.0"
    LOG_DIR="$PROJECT_ROOT/results/intervention/logs"
    
    # 模型与SAE配置（保持原始2B-PT，与run_get_sae_act_gemma_2b_pt.sh保持一致）
    MODEL_NAME="google/gemma-2-2b"
    SAE_RELEASE="gemma-scope-2b-pt-res"
    SAE_LOCAL_BASE_DIR="/path/to/local_models/gemma-scope-2b-pt-res"
    
    print_info "Parcel Intervention System 启动"
