...
# Parcel Intervention System 运行脚本（9b-it + instruct）
# 用于执行第三步的因果干预实验
...
run_single_dataset() {
    ...
    local log_file="$LOG_DIR/${dataset_name}_intervention_9b.log"
...
run_all_datasets() {
    ...
    local log_file="$LOG_DIR/all_datasets_intervention_9b.log"
    local timestamp=$(date '+%Y%m%d_%H%M%S')
...
# 显示帮助信息
show_help() {
    echo "Parcel Intervention System 运行脚本（9b-it + instruct）"
    echo ""
    echo "用法: $0 [选项]"
    echo ""
    echo "选项:"
...
main() {
    ...
    DATA_DRIVEN_RESULTS="$PROJECT_ROOT/results/rank_activation_9b/parcel_activation_rankings.json"
    LATENT_PARCEL_ASSIGNMENTS="/path/to/project_root/neural_area/divide_area_by_sae_act/cluster_output_9b_it/clustering_results_sentence_prep0.03_0.8_svdvar0p80_parcels20_iter50_spatial0.01_nparcels270/latent_parcel_assignments.json"
    MAX_ACTIVATION_DIR="$PROJECT_ROOT/results/steer_activation"
    RESULTS_DIR="$PROJECT_ROOT/results/intervention/strength_0.5_9b"
