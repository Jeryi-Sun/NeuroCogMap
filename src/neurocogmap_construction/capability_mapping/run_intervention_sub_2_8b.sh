...
# 等待上游进程结束后，启动 Llama-3.1-8B 的 Parcel Intervention 运行脚本
...
echo "Process $TARGET_PID finished. Starting 8b intervention..."

# 进入脚本目录并调用 8b 运行脚本
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

# 示例：处理所有数据集前 50 个 parcel
# 使用: bash run_intervention_sub_1_8b.sh

bash "$SCRIPT_DIR/run_intervention_8b.sh" --all --top-k 50
