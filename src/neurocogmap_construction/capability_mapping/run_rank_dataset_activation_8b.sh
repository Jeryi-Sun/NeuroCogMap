...
# Capability-Parcel Connection Analysis Runner (Llama-3.1-8B + LXR-8x)
# 针对 Llama-3.1-8B Base + LXR-8x SAE 的能力-分区连接分析运行脚本
...
# 显示帮助信息
show_help() {
    echo "Capability-Parcel Connection Analysis Runner (Llama-3.1-8B + LXR-8x)"
    echo "=================================================================="
    echo ""
    echo "用法: $0 [选项]"
    echo ""
    ...
    echo "  -k, --top-k             数据驱动分析中的top_k参数 [默认: 50]"
    echo "  -o, --output            结果输出目录"
    echo "  --capability-data       能力数据集统计文件路径"
    echo "  --parcel-assignments    parcel分配结果文件路径（8B SAE 聚类结果）"
    echo "  --sae-output            SAE激活数据目录路径（8B SAE 激活）"
    echo ""
    echo "示例:"
    echo "  $0 -d sentence -n l2 -k 50"
    echo "  $0 --data-level example --normalize zscore --top-k 20"
    echo ""
}

# 默认参数（在 9b 和 2b 版本基础上改为 8B 配置）
DATA_LEVEL="sentence"
NORMALIZE_METHOD="l2"
TOP_K=50
RESULTS_DIR="/path/to/project_root/neural_area/connect_cap_parcel/results/rank_activation_8b"
...
PARCEL_ASSIGNMENTS="/path/to/project_root/neural_area/divide_area_by_sae_act/cluster_output_llama_8b_pt/clustering_results_sentence_prep0.01_0.8_svdvar0p80_parcels20_iter50_spatial0.01_nparcels240/latent_parcel_assignments.json"
SAE_OUTPUT="/path/to/project_root/neural_area/divide_area_by_sae_act/qa_sae_output_llama_8b_pt"
...
main() {
    echo "=================================================================="
    echo "Capability-Parcel Connection Analysis (Llama-3.1-8B + LXR-8x)"
    echo "能力-分区连接分析系统（Llama-3.1-8B）"
    echo "=================================================================="
    echo ""
