...
# ============================================================
# 功能介绍：
#   为 fairness_bias 结果中的 parcel-level anomalous_connections.json
...
#
# 用法：
#   bash safety_explanation/fairness_bias/analysis_graphs/run_annotate_struct_sign_in_anomalous_connections.sh
#
# 说明：
#   - DATASETS：可配置要处理的模型数据集（与 run_circle_graph_edge2node.sh 风格一致）
#   - ANALYSIS_OUT：fairness_bias 结果根目录（每个 DATASET 一个子目录）
...
# 结构连接矩阵 CSV 路径（与 circle_graph_edge2node.py 中默认保持一致）
...
for DATASET in "${DATASETS[@]}"; do
  ROOT_RESULTS_DIR="${ANALYSIS_OUT}/${DATASET}"
...
echo "[INFO] 所有指定数据集的结构连接正负标记批处理完成。"