#!/bin/bash
# Fairness/Bias 机制分析运行脚本（Llama-3.1-8B base）
#
# 使用方法:
#   bash run_analysis_bias_8b.sh                    # 运行完整分析（所有模型数据）
#   bash run_analysis_bias_8b.sh check              # 检查环境
#   修改 RUN_MODE 为 parcel / capability / llm 可切换分析阶段
#
# 分析功能（8B 版本）:
#   - 支持批量处理 BBQ 数据集（bbq_*_Llama-3.1-8B）
...
MODEL_DATA_LIST=(
"bbq_age_Llama-3.1-8B"
"bbq_disability_status_Llama-3.1-8B"
"bbq_gender_identity_Llama-3.1-8B"
"bbq_nationality_Llama-3.1-8B"
)
...
# 运行完整分析
run_full_analysis() {
    print_info "运行完整 Fairness/Bias 机制分析（8B）..."
...
# 命令行可覆盖 RUN_MODE，例如: bash run_analysis_bias_8b.sh parcel
if [ -n "${1:-}" ]; then
    RUN_MODE="$1"
fi
