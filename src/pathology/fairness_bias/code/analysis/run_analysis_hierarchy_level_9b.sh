...
# 前置：请先在公平偏见任务上运行对应的 9B 整体分析脚本（如 run_analysis_bias_9b.sh 或 analysis_capability_level.py），生成：
#   <BASE_DIR>/safety_explanation/fairness_bias/results/analysis_output/<MODEL_DATA>/capability_level/capability_level_analysis_complete.json
#
# 用法示例：
...
# 模型数据列表 - 与 fairness_bias 下的 run_analysis_bias_9b.sh 对齐
MODEL_DATA_LIST=(
...
for MODEL_DATA in "${MODEL_DATA_LIST[@]}"; do
  MODEL_OUTPUT_DIR="$BASE_DIR/safety_explanation/fairness_bias/results/analysis_output/$MODEL_DATA"
