...
# 结果输出根目录
DETECT_OUT_DIR="$BASE_DIR/safety_explanation/fairness_bias/results/detection"
# 分析输出根目录（用于自动构建指示器配置）
ANALYSIS_OUT_ROOT="$BASE_DIR/safety_explanation/fairness_bias/results/analysis_output"
...
run_detection() {
    ...
    for MODEL_DATA in "${MODEL_DATA_LIST[@]}"; do
        ...
        CORRECT_JSONL="$BASE_DIR/safety_explanation/fairness_bias/results/$MODEL_DATA/parcels_token_acts/correct/token_parcels.jsonl"
        INCORRECT_JSONL="$BASE_DIR/safety_explanation/fairness_bias/results/$MODEL_DATA/parcels_token_acts/incorrect/token_parcels.jsonl"
        ...
        if [ $? -eq 0 ]; then
            ...
            print_info "- $OUT_DIR/fairness_bias_detector.joblib"
        else
