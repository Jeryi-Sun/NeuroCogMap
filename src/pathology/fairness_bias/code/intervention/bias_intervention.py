...
基于幻觉干预系统的实现，对参数进行适配，便于在 fairness/bias 数据集上复用。

主要改动：
- 复用 `HallucinationIntervention` 主类
- 调整默认目录（analysis/output、results、dataset 等）为 fairness_bias 相关路径
- 保持与幻觉版本一致的接口
"""
...
# 从 fairness_bias/code/intervention 向上三级到 safety_explanation，然后进入 hallucination/code/intervention
...
class BiasIntervention(HallucinationIntervention):
    """Fairness/Bias 场景专用干预系统（复用幻觉版本的实现）"""
    ...
    def __init__(
        ...
        parcel_json_path: str = "/path/to/project_root/safety_explanation/fairness_bias/results/analysis_output/bbq_age_gemma-2-2b/parcel_level/top_anomalous_parcels.json",
        ...
        results_dir: str = "/path/to/project_root/safety_explanation/fairness_bias/results/intervention",
