...
# -*- coding: utf-8 -*-
"""
Fairness/Bias 干预运行脚本

参考 jailbreak 的干预脚本，支持单实验与批量实验两种模式，并支持直接保存 JSON 结果文件。
"""
...
# 导入 Bias 专用实现
from data_loader import BiasDataLoader, BiasEvaluator
from bias_intervention import BiasIntervention


def setup_logging(log_level: str = "INFO") -> logging.Logger:
    logger = logging.getLogger("bias_intervention")
...
def run_single_experiment(config: Dict[str, Any],
    ...
    intervention = BiasIntervention(
        ...
        results_dir=config.get("results_dir", "/path/to/project_root/safety_explanation/fairness_bias/results/intervention"),
    ...
    data_loader = BiasDataLoader(logger)
    ...
    if enable_evaluation and config.get("vllm_url"):
        ...
        try:
            ...
            if "intervention_results" in intervention_results:
                ...
                logger.info(f"评估完成 - 基线偏见率: {eval_results['baseline_stats']['bias_rate']:.3f}%, "
                           f"干预偏见率: {eval_results['intervention_stats']['bias_rate']:.3f}%, "
                           f"改善: {eval_results['improvement']['bias_rate_diff']:.1f}%")
            else:
        ...
        logger.warning("未配置 vLLM 地址，跳过评估")
...
def main():
    parser = argparse.ArgumentParser(description="Fairness/Bias 干预实验运行脚本")
