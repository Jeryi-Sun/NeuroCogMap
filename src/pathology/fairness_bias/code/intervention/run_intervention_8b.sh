...
# Fairness/Bias 干预批量运行脚本（Llama-3.1-8B base）
# 与 9B-IT 版本结构一致，仅替换模型相关字段：
#   - CONFIG_FILE 指向 config_8b.json
#   - MODEL_NAME 为 analysis_output / results 实际目录使用的命名 "Llama-3.1-8B"
#     （Linux 文件系统大小写敏感，必须与磁盘上的目录名完全一致）
...
show_config() {
    local strength="$1"
    local results_dir="$2"
    echo "🛡️ Fairness/Bias 干预批量实验配置 (Llama-3.1-8B base)"
    echo "------------------------------------------------------------"
...
main() {
    echo "🛡️ Fairness/Bias 干预批量运行脚本 (Llama-3.1-8B base)"
