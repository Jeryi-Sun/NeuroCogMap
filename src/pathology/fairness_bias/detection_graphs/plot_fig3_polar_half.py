"""
Bias 数据极坐标检测性能图（1/2 圆版本）。
完全按照提供的半圆极坐标代码绘制，仅使用 BBQ-* 数据集。
"""
import argparse
import ast
...
BASE_DIR = Path('/path/to/project_root/safety_explanation/fairness_bias')
CSV_PATH = BASE_DIR / 'results/detection/all_results/all_metrics_flat.csv'
OUT_DIR = BASE_DIR / 'detection_graphs/output'
DEFAULT_METRIC = 'auroc'
BIAS_DATASETS = {'BBQ-Age', 'BBQ-Nationality', 'BBQ-Gender', 'BBQ-Disability'}
...
def load_df(csv_path=None, metric=DEFAULT_METRIC):
    ...
    out = out[out['dataset'].isin(BIAS_DATASETS)].copy()
    if out.empty:
        raise ValueError('No bias datasets (BBQ-*) found in CSV.')
    return out
