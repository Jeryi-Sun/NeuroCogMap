...
# ============================================================
# 功能介绍：
#   为 fairness_bias 结果绘制平均检测性能柱状图，复用 hallucination
#   下的 plot_fig3_average_only.py。
#
# 用法示例：
#   bash safety_explanation/fairness_bias/detection_graphs/run_plot_fig3_average_only.sh
#   METRIC=auprc bash safety_explanation/fairness_bias/detection_graphs/run_plot_fig3_average_only.sh
...
python3 "$SCRIPT_PATH" \
    ...
    --out-dir "$OUT_DIR" \
    --metric "$METRIC" \
    2>&1 | tee "$BASE_DIR/safety_explanation/fairness_bias/detection_graphs/run_plot_fig3_average_only.log"
