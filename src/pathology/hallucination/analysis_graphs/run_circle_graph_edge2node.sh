#!/usr/bin/env bash
# 简易脚本：调用 circle_graph_edge2node.py 绘制
# "根据异常 edge 选择 node"的 Parcel/Capability-level circle graph。
# 现在改为在脚本内部固定 for 循环依次处理若干数据集，无需外部传参。
#
# 使用方式：
#   bash safety_explanation/hallucination/analysis_graphs/run_circle_graph_edge2node.sh
#
# 说明：
...
# 级别类型：parcel 或 capability（默认 capability）
LEVEL="parcel"

# edge 选择策略：abs（默认，按绝对值整体排序）或 half_signed（正负各取一半）
...
for DATASET in "${DATASETS[@]}"; do
  ANOMALOUS_EDGES_JSON="${ROOT_DIR}/safety_explanation/hallucination/results/analysis_output/${DATASET}/${LEVEL_NAME}/anomalous_connections.json"
  ANALYSIS_COMPLETE_JSON="${ROOT_DIR}/safety_explanation/hallucination/results/analysis_output/${DATASET}/${LEVEL_NAME}/${LEVEL_NAME}_analysis_complete.json"

  DATA_DIR="${ROOT_DIR}/safety_explanation/hallucination/analysis_graphs/data/${DATASET}/${LEVEL_NAME}"
  FIG_DIR="${ROOT_DIR}/safety_explanation/hallucination/analysis_graphs/figures/${DATASET}/${LEVEL_NAME}"
  ...
  python "${ROOT_DIR}/safety_explanation/hallucination/analysis_graphs/circle_graph_edge2node.py" "${PYTHON_ARGS[@]}"

  echo "[INFO] 完成: ${DATASET}"
  echo ""
done

echo "[INFO] 所有指定数据集的 circle graph 已完成。"
