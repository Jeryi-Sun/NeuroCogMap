...
#
# 分析功能（8B 版本）:
#   - 支持批量处理多个模型数据（MedHallu_Llama-3.1-8B, HaluEval_Llama-3.1-8B等）
...
# 模型数据列表 - 支持批量处理多个模型（8B 版本）
MODEL_DATA_LIST=(
    "MedHallu_Llama-3.1-8B"
    "HaluEval_Llama-3.1-8B"
    "dolly_close_Llama-3.1-8B"
    "nq_open_Llama-3.1-8B"
    "sciq_Llama-3.1-8B"
    "triviaqa_Llama-3.1-8B"
    "truthfulqa_Llama-3.1-8B"
)
...
# 结构性连接mask参数
USE_STRUCTURAL_MASK=false  # 是否使用结构性连接矩阵作为mask
...
check_environment() {
    ...
    for MODEL_DATA in "${MODEL_DATA_LIST[@]}"; do
        local correct_file="$BASE_DIR/safety_explanation/hallucination/results/$MODEL_DATA/parcels_token_acts/correct/token_parcels.jsonl"
        local incorrect_file="$BASE_DIR/safety_explanation/hallucination/results/$MODEL_DATA/parcels_token_acts/incorrect/token_parcels.jsonl"
    ...
    if [ "$has_model_data" = false ]; then
        ...
        for MODEL_DATA in "${MODEL_DATA_LIST[@]}"; do
            print_error "  - $BASE_DIR/safety_explanation/hallucination/results/$MODEL_DATA/parcels_token_acts/correct/token_parcels.jsonl"
            print_error "  - $BASE_DIR/safety_explanation/hallucination/results/$MODEL_DATA/parcels_token_acts/incorrect/token_parcels.jsonl"
        done
        exit 1
    fi
...
run_full_analysis() {
    ...
    for MODEL_DATA in "${MODEL_DATA_LIST[@]}"; do
        ...
        # 为每个模型数据设置路径
        CORRECT_ACTIVATIONS="$BASE_DIR/safety_explanation/hallucination/results/$MODEL_DATA/parcels_token_acts/correct/token_parcels.jsonl"
        INCORRECT_ACTIVATIONS="$BASE_DIR/safety_explanation/hallucination/results/$MODEL_DATA/parcels_token_acts/incorrect/token_parcels.jsonl"
        OUTPUT_DIR="$BASE_DIR/safety_explanation/hallucination/results/analysis_output/$MODEL_DATA"
...
run_parcel_analysis() {
    ...
    for MODEL_DATA in "${MODEL_DATA_LIST[@]}"; do
        ...
        # 构建Parcel级别分析命令
        PARCEL_CMD="python3 analysis_parcel_level.py \
            --correct_jsonl \"$CORRECT_ACTIVATIONS\" \
            --incorrect_jsonl \"$INCORRECT_ACTIVATIONS\" \
...
run_capability_analysis() {
    ...
    for MODEL_DATA in "${MODEL_DATA_LIST[@]}"; do
        ...
        # 构建Capability级别分析命令
        CAPABILITY_CMD="python3 analysis_capability_level.py \
            --mapping_json \"$MAPPING_JSON\" \
            --correct_jsonl \"$CORRECT_ACTIVATIONS\" \
            --incorrect_jsonl \"$INCORRECT_ACTIVATIONS\" \
...
run_llm_report() {
    ...
    for MODEL_DATA in "${MODEL_DATA_LIST[@]}"; do
        ...
        python3 analysis_llm_summary.py \
            --parcel_desc "$PARCEL_DESC" \
            --cap_desc "$CAP_DESC" \
