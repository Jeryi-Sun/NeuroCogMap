...
# 与 analysis_output 子目录命名一致：{dataset}_Llama-3.1-8B（见 run_analysis_overall_8b.sh）
MODEL_NAME="Llama-3.1-8B"                      # 用于结果文件名与异常分析路径拼接
...
run_all_experiments() {
    ...
    local csv_files=($(find "$DATASET_DIR" -name "*.csv" -type f 2>/dev/null | grep -E "(MedHallu|truthfulqa|nq_open)" 2>/dev/null | sort || true))
