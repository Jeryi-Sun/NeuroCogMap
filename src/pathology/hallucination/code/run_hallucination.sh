...
# 模型配置
MODEL_ID="google/gemma-2-9b-it"  # 或者 google/gemma-2-2b

# 数据集路径（支持单个文件或目录）
# 如果是目录，将处理目录下所有.csv文件
CSV_PATH="/path/to/project_root/safety_explanation/hallucination/dataset"

# 输出目录
OUTPUT_DIR="/path/to/project_root/safety_explanation/hallucination/results"
...
PY_SCRIPT="$SCRIPT_DIR/hallucination_generate_and_eval.py"
...
process_csv() {
  ...
  if [[ $? -eq 0 ]]; then
    echo "[SUCCESS] $csv_basename 处理完成"
  else
    echo "[ERROR] $csv_basename 处理失败" >&2
  fi
}
