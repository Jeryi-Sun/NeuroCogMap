#!/usr/bin/env bash
# 功能：运行 plot_fig3_polar_only.py 生成极坐标图
#
# 用法示例：
#   bash run_plot_fig3_polar_only.sh
#   METRIC=auprc bash run_plot_fig3_polar_only.sh
...
python3 "$SCRIPT_PATH" \
    ...
    --out-dir "$OUT_DIR" \
    --metric "$METRIC" \
    2>&1 | tee "$BASE_DIR/safety_explanation/hallucination/detection_graphs/run_plot_fig3_polar_only.log"
