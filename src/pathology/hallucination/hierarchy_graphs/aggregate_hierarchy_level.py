...
输入来源：
  <BASE_DIR>/safety_explanation/hallucination/results/analysis_output/<MODEL_DATA>/hierarchy_level/hierarchy_level_activation_diff.json

输出：
  <BASE_DIR>/safety_explanation/hallucination/hierarchy_graphs/data/hierarchy_level_all_models.json

结构示例：
{
  "dolly_close_gemma-2-2b": { ... 原始层级 JSON ... },
  "triviaqa_gemma-2-2b": { ... },
  ...
}
"""
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="项目类型：hallucination 或 fairness_bias（默认: hallucination）",
    )
    parser.add_argument(
        "--model-data-list",
        type=str,
        ...
        help="需要汇总的模型列表（与 run_analysis_overall.sh / run_analysis_hierarchy_level.sh 对齐）。",
    )
    parser.add_argument(
        "--output-json",
        type=str,
        ...
        help=(
            "汇总输出 JSON 路径，"
            "默认: <base-dir>/safety_explanation/hallucination/hierarchy_graphs/data/hierarchy_level_all_models.json"
        ),
    )
    parser.add_argument(
        "--skip-existing",
