...
# ============================================================
# 功能介绍：
#   为 jailbreak 结果绘制 activation_diff 条形图，复用 hallucination
#   下的 bar_graph.py，通过 --group_high_label / --group_low_label 传入
#   Correct / Incorrect 标签。
#   支持 parcel / capability 两种级别（脚本内 LEVEL 变量）。
#
# 用法示例：
#   bash safety_explanation/jailbreak/analysis_graphs/run_bar_graph.sh
#
# 说明：
...
FORCE_RECOMPUTE="" # 如需强制重写，改为 "--force_recompute"
...
ANALYSIS_DIR="${ROOT_DIR}/safety_explanation/jailbreak/results/analysis_output"
...
# 图例与 Y 轴标签：Correct vs Incorrect（jailbreak 语境）
...
process_dataset() {
    ...
    # 缓存数据路径
    CACHE_DATA_DIR="${ROOT_DIR}/safety_explanation/jailbreak/analysis_graphs/data/${ds}/${LEVEL_NAME}"
