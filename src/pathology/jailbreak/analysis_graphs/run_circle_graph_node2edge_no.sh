...
# ============================================================
# 功能介绍：
#   为 jailbreak 结果绘制 Parcel 级 circle graph（由 top parcels 选节点，
#   再画连接边），复用 hallucination 下 circle_graph_node2edge_no.py，
#   使用 Correct / Incorrect 标签。
#
# 用法：
#   bash safety_explanation/jailbreak/analysis_graphs/run_circle_graph_node2edge_no.sh
# 或指定数据集与方法：
#   bash .../run_circle_graph_node2edge_no.sh JBB-Behaviors_gemma-2-9b-it traditional
#
# 说明：
...
run_one() {
    ...
    DATA_DIR="${ROOT_DIR}/safety_explanation/jailbreak/analysis_graphs/data/${DATASET}/${LEVEL_NAME}"
    FIG_DIR="${ROOT_DIR}/safety_explanation/jailbreak/analysis_graphs/figures/${DATASET}/${LEVEL_NAME}"
