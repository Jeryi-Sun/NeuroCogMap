#!/bin/bash
# Jailbreak 机制分析运行脚本（Llama-3.1-8B base）
#
# 使用方法:
#   bash run_analysis_jailbreak_8b.sh                    # 运行完整分析（所有模型数据）
#   bash run_analysis_jailbreak_8b.sh check              # 检查环境
#   修改 RUN_MODE 为 parcel / capability / llm 可切换分析阶段
#
# 分析功能（8B 版本）:
#   - 支持批量处理 AdvBench / JBB-Behaviors（*_Llama-3.1-8B）
...
# 模型数据列表 - 支持批量处理多个模型（8B 版本，目录名大小写须与磁盘一致）
MODEL_DATA_LIST=(
"AdvBench_Llama-3.1-8B"
"JBB-Behaviors_Llama-3.1-8B"
)

# 数据文件路径（全局共享文件，8B 版本）
MAPPING_JSON="$BASE_DIR/neural_area/connect_cap_parcel/results/aggrate_final_8b/final_capability_parcel_all.json"
PARCEL_DESC="$BASE_DIR/neural_area/divide_area_by_sae_act/cluster_output_llama_8b_pt/clustering_results_sentence_prep0.01_0.8_svdvar0p80_parcels20_iter50_spatial0.01_nparcels240/latent_parcel_topsamples_functionality_summary.json"
...
# 结构性连接mask参数
USE_STRUCTURAL_MASK=false  # 是否使用结构性连接矩阵作为mask（8B parcel 数与 2B 结构性矩阵不一致时保持 false）
...
check_environment() {
    ...
    for MODEL_DATA in "${MODEL_DATA_LIST[@]}"; do
        local correct_file="$BASE_DIR/safety_explanation/jailbreak/results/$MODEL_DATA/parcels_token_acts/correct/token_parcels.jsonl"
        local incorrect_file="$BASE_DIR/safety_explanation/jailbreak/results/$MODEL_DATA/parcels_token_acts/incorrect/token_parcels.jsonl"
    ...
    if [ "$has_model_data" = false ]; then
        ...
        for MODEL_DATA in "${MODEL_DATA_LIST[@]}"; do
            print_error "  - $BASE_DIR/safety_explanation/jailbreak/results/$MODEL_DATA/parcels_token_acts/correct/token_parcels.jsonl"
            print_error "  - $BASE_DIR/safety_explanation/jailbreak/results/$MODEL_DATA/parcels_token_acts/incorrect/token_parcels.jsonl"
        done
        exit 1
    fi
...
# 运行完整分析
run_full_analysis() {
    print_info "运行完整 Jailbreak 机制分析（8B）..."
    ...
    for MODEL_DATA in "${MODEL_DATA_LIST[@]}"; do
        ...
        # 为每个模型数据设置路径
        CORRECT_ACTIVATIONS="$BASE_DIR/safety_explanation/jailbreak/results/$MODEL_DATA/parcels_token_acts/correct/token_parcels.jsonl"
        INCORRECT_ACTIVATIONS="$BASE_DIR/safety_explanation/jailbreak/results/$MODEL_DATA/parcels_token_acts/incorrect/token_parcels.jsonl"
        OUTPUT_DIR="$BASE_DIR/safety_explanation/jailbreak/results/analysis_output/$MODEL_DATA"
...
# 命令行可覆盖 RUN_MODE，例如: bash run_analysis_jailbreak_8b.sh parcel
if [ -n "${1:-}" ]; then
    RUN_MODE="$1"
fi
