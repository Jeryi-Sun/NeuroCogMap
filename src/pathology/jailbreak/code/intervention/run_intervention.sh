...
# Jailbreak 干预实验批量运行脚本（Gemma-2-2b）
...
check_dataset_dir() {
    ...
    echo "[INFO] 找到 ${#csv_files[@]} 个 CSV 文件"
}
...
run_single_experiment() {
    ...
    if [[ "$SKIP_EXISTING" == "yes" ]]; then
        ...
        if [[ -f "$result_file" ]]; then
            echo "⏭️  已存在结果，跳过: $(basename "$result_file")"
            return 0
        fi
    fi
