...
# Jailbreak 干预批量运行脚本（Gemma-2-9b-it）
...
show_config() {
    local strength="$1"
    local results_dir="$2"
    echo "🛡️ Jailbreak 干预批量实验配置 (9B-IT)"
    echo "------------------------------------------------------------"
...
check_vllm_service() {
    ...
    if command -v curl &> /dev/null; then
        ...
        # 如果没有 curl，尝试使用 wget
        local wget_output=$(wget --spider --timeout=3 --tries=1 "$health_url" 2>&1)
        if [[ $? -eq 0 ]]; then
            echo "[SUCCESS] vLLM 服务运行正常: $VLLM_URL"
            return 0
        else
...
main() {
    echo "🛡️ Jailbreak 干预批量运行脚本 (9B-IT)"
