...
SCRIPT_PATH="$BASE_DIR/safety_explanation/jailbreak/detection_graphs/plot_fig3_polar_half.py"

python3 "$SCRIPT_PATH" \
    --csv-path "$CSV_PATH" \
    --out-dir "$OUT_DIR" \
    --metric "$METRIC" \
    2>&1 | tee "$BASE_DIR/safety_explanation/jailbreak/detection_graphs/run_plot_fig3_polar_half.log"
