...
# ============================================================
# 功能介绍：
#   为 sycophancy 结果绘制"由异常 edge 选 node"的 Parcel/Capability 级
#   circle graph，复用 hallucination 下 circle_graph_edge2node.py，
#   使用 Correct / Incorrect 标签。
#
# 用法：
#   bash safety_explanation/sycophancy/analysis_graphs/run_circle_graph_edge2node.sh
#
# 说明：
...
#   - Edge 数量设置（二选一）：
#     * 方式1：使用 MAX_EDGES（默认）
#     * 方式2：分别设置 POS_EDGES 和 NEG_EDGES（与 MAX_EDGES 互斥）
...
# Edge 选择方式（二选一）：
#   方式1：使用 max_edges（与 pos_edges/neg_edges 互斥）
# MAX_EDGES=20
#   方式2：分别指定 pos 和 neg 的数量（与 max_edges 互斥）
POS_EDGES=5
NEG_EDGES=7
...
DATA_BASE="${ROOT_DIR}/safety_explanation/sycophancy/analysis_graphs/data"
...
if [ -n "${POS_EDGES:-}" ] && [ -n "${NEG_EDGES:-}" ]; then
  echo "[INFO] Edge 数量设置: pos=${POS_EDGES}, neg=${NEG_EDGES}"
else
  echo "[INFO] Edge 数量设置: max_edges=${MAX_EDGES}"
fi
...
for DATASET in "${DATASETS[@]}"; do
  ...
  # 在文件名中编码 edge_selection 和 edge 数量参数，方便同时保存多种 edge 选择策略的结果
  if [ -n "${POS_EDGES:-}" ] && [ -n "${NEG_EDGES:-}" ]; then
    EDGE_COUNT_SUFFIX="pos${POS_EDGES}_neg${NEG_EDGES}"
  else
    EDGE_COUNT_SUFFIX="max${MAX_EDGES}"
  fi
  OUTPUT_DATA="${DATA_DIR}/circle_graph_edge2node_${EDGE_SELECTION}_${EDGE_COUNT_SUFFIX}.json"
  OUTPUT_FIG="${FIG_DIR}/circle_graph_edge2node_${EDGE_SELECTION}_${EDGE_COUNT_SUFFIX}.pdf"
  ...
  if [ -n "${POS_EDGES:-}" ] && [ -n "${NEG_EDGES:-}" ]; then
    echo "[INFO] POS_EDGES             = ${POS_EDGES}"
    echo "[INFO] NEG_EDGES             = ${NEG_EDGES}"
  else
    echo "[INFO] MAX_EDGES             = ${MAX_EDGES}"
  fi
  ...
  # 根据是否设置了 pos_edges/neg_edges 来决定使用哪种方式
  if [ -n "${POS_EDGES:-}" ] && [ -n "${NEG_EDGES:-}" ]; then
    PYTHON_ARGS+=(--pos_edges "${POS_EDGES}")
    PYTHON_ARGS+=(--neg_edges "${NEG_EDGES}")
  else
    PYTHON_ARGS+=(--max_edges "${MAX_EDGES}")
  fi
