#!/bin/bash
# Sycophancy 机制分析运行脚本（Llama-3.1-8B base）
#
# 使用方法:
#   bash run_analysis_sycophancy_8b.sh                    # 运行完整分析（所有模型数据）
#   bash run_analysis_sycophancy_8b.sh check              # 检查环境
#   修改 RUN_MODE 为 parcel / capability / llm 可切换分析阶段
#
# 分析功能（8B 版本）:
#   - 支持批量处理 answer / feedback（*_Llama-3.1-8B）
...
# 运行完整分析
run_full_analysis() {
    print_info "运行完整 Sycophancy 机制分析（8B）..."
    ...
    for MODEL_DATA in "${MODEL_DATA_LIST[@]}"; do
        ...
        # 为每个模型数据设置路径
        CORRECT_ACTIVATIONS="$BASE_DIR/safety_explanation/sycophancy/results/$MODEL_DATA/parcels_token_acts/correct/token_parcels.jsonl"
        INCORRECT_ACTIVATIONS="$BASE_DIR/safety_explanation/sycophancy/results/$MODEL_DATA/parcels_token_acts/incorrect/token_parcels.jsonl"
        OUTPUT_DIR="$BASE_DIR/safety_explanation/sycophancy/results/analysis_output/$MODEL_DATA"
...
# 命令行可覆盖 RUN_MODE，例如: bash run_analysis_sycophancy_8b.sh parcel
if [ -n "${1:-}" ]; then
    RUN_MODE="$1"
fi
