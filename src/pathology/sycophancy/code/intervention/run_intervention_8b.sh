...
# Sycophancy 干预批量运行脚本（Llama-3.1-8B base）
# 与 9B-IT 版本结构一致，仅替换模型相关字段：
#   - CONFIG_FILE 指向 config_8b.json
#   - MODEL_NAME 严格匹配磁盘上实际目录命名 "Llama-3.1-8B"，用于：
#       * 输入 jsonl: ${DATA_TYPE}_with_groups_${MODEL_NAME}_gen.jsonl
#       * 异常分析: ${ANALYSIS_DIR}/${DATA_TYPE}_${MODEL_NAME}/parcel_level/top_anomalous_parcels.json
#       * 结果文件: ${MODEL_NAME}_${DATA_TYPE}_intervention.json
# 同时处理 feedback（如需 answer 一起跑，将 DATA_TYPES 改为 (answer feedback)）。
...
# 数据源类型：与 2B/9B 保持一致，默认只跑 feedback
...
# 当前处理的分析文件（由 main 中按 DATA_TYPE 设置）
ANALYSIS_FILE=""
...
extract_parcel_ids() {
    local analysis_file="$ANALYSIS_FILE"
...
show_config() {
    ...
    local data_type="$3"
    local input_file="$4"
    echo "🧩 Sycophancy 干预批量实验配置 (Llama-3.1-8B base)"
    echo "------------------------------------------------------------"
    echo "数据源类型:   $data_type"
    echo "输入文件:     $input_file"
    echo "异常分析文件: $ANALYSIS_FILE"
...
run_all_experiments() {
    ...
    run_single_experiment "$input_file" "$selected_strength" "$results_dir" "$data_type"
    echo ""
    echo "🎉 Llama-3.1-8B Sycophancy [$data_type] 干预完成 (强度: $selected_strength)，结果目录: $results_dir"
    return 0
}

main() {
    echo "🧩 Sycophancy 干预批量运行脚本 (Llama-3.1-8B base) — answer + feedback"
    ...
    for STRENGTH in "${STRENGTH_LIST[@]}"; do
        ...
        for DATA_TYPE in "${DATA_TYPES[@]}"; do
            INPUT_FILE="$(get_input_file_for_data_type "$DATA_TYPE")"
            ANALYSIS_FILE="$ANALYSIS_DIR/${DATA_TYPE}_${MODEL_NAME}/parcel_level/top_anomalous_parcels.json"
            show_config "$STRENGTH" "$RESULTS_DIR" "$DATA_TYPE" "$INPUT_FILE"
            check_input_file "$INPUT_FILE" || continue
            run_all_experiments "$STRENGTH" "$RESULTS_DIR" "$DATA_TYPE" || true
        done
        echo ""
