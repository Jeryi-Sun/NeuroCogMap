...
# Sycophancy 干预批量运行脚本（Gemma-2-9b-it）
...
show_config() {
    ...
    local data_type="$3"
    local input_file="$4"
    echo "🧩 Sycophancy 干预批量实验配置 (9B-IT)"
    echo "------------------------------------------------------------"
...
run_all_experiments() {
    ...
    echo ""
    echo "🎉 9B-IT Sycophancy [$data_type] 干预完成 (强度: $selected_strength)，结果目录: $results_dir"
    return 0
}

main() {
    echo "🧩 Sycophancy 干预批量运行脚本 (9B-IT) — answer + feedback"
