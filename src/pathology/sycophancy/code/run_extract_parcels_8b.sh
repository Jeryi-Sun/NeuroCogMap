...
# 功能: 针对 Llama-3.1-8B 的 sycophancy 结果抽取 parcel-level token activations（按 combo 循环）。
...
# 根据 results 下的目录命名调整（常见: answer_<model> / feedback_<model>）
COMBO_NAMES=(
...
# Llama-3.1-8B 对应的 parcel 映射（与其它 8B 脚本保持一致）
