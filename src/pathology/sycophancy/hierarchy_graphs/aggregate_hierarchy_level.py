...
输入来源：
  <BASE_DIR>/safety_explanation/sycophancy/results/analysis_output/<MODEL_DATA>/hierarchy_level/hierarchy_level_activation_diff.json

输出：
  <BASE_DIR>/safety_explanation/sycophancy/hierarchy_graphs/data/hierarchy_level_all_models.json

结构示例：
{
  "answer_gemma-2-2b": { ... 原始层级 JSON ... },
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help=(
            "汇总输出 JSON 路径，"
            "默认: <base-dir>/safety_explanation/sycophancy/hierarchy_graphs/data/hierarchy_level_all_models.json"
        ),
    )
    parser.add_argument(
        "--skip-existing",
